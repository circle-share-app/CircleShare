...
interface ICircle {
    ...
    event PermissionAssigned(address memberAddress, Storage.Permission permission);
    event PermissionRevoked(address memberAddress, Storage.Permission permission);
    ...
    event InvitationSent(address inviter, address invitee, string nickname, uint256 timestamp);
    ...
    function getMembers() external view returns (Storage.Member[] memory members);
    ...
    function assignPermission(address memberAddress, Storage.Permission permission) external;

    function revokePermission(address memberAddress, Storage.Permission permission) external;

    function getRole(address memberAddress) external view returns (Storage.Role role);

    function isMember(address memberAddress) external view returns (bool);

    function hasPermission(address memberAddress, Storage.Permission permission) external view returns (bool);

    function deactivateCircle() external;

    function reactivateCircle() external;

    function inviteMember(address invitee, string memory nickname) external;

    function acceptInvitation() external;

    function rejectInvitation() external;

    function addExpense(string memory description, uint256 amount, address[] memory participants) external;

    function getExpenses() external view returns (Storage.Expense[] memory);

    function getExpense(uint256 expenseId) external view returns (Storage.Expense memory);

    function getPendingInvitations() external view returns (Storage.Invitation[] memory);
